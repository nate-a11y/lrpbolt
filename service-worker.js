...
precacheAndRoute([...manifest, { url: 'manifest.webmanifest', revision: null }]);
...
registerRoute(
  ...
  async (options) => {
    try {
      return await new CacheFirst({ cacheName: 'assets-cache' }).handle(options);
    } catch (err) {
      return new StaleWhileRevalidate({ cacheName: 'assets-cache' }).handle(options);
    }
  }
);
