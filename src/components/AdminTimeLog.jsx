...
export default function AdminTimeLog() {
  ...
  const detectIssues = (entries) => {
    ...
    Object.values(grouped).forEach((arr) => {
      const sorted = arr.sort(
        (a, b) => dayjs(a.rawStart).valueOf() - dayjs(b.rawStart).valueOf(),
      );
      for (let i = 0; i < sorted.length; i++) {
        ...
        if (!end.isValid()) {
          res.push({
            ...r,
            type: "Missing End Time",
            severity: "critical",
            details: "Driver did not clock out",
          });
        } else {
          ...
          if (dur > thresh) {
            res.push({
              ...
              type: "Long Duration",
              severity: "warning",
              details: `Duration ${durationFormat(dur)}`,
            });
          }
  ...
  const toggleAll = () => {
    const next = baseColumns.reduce(
      (acc, c) => ({ ...acc, [c.field]: !allSelected }),
      {},
    );
                ...
                <TableBody>
                  ...
                  {summary.length > 0 && (
                    <TableRow sx={{ fontWeight: "bold" }}>
                      ...
                      <TableCell>
                        {summaryTotals.TotalHours.toFixed(2)}
                      </TableCell>
                      <TableCell>{summaryTotals.TotalRides}</TableCell>
                      <TableCell>
                        {summaryTotals.AvgDuration.toFixed(2)}
                      </TableCell>
