...
import {
  Box, Card, CardContent, CardHeader, Typography, Stack, Chip, Tooltip,
...
const Stat = ({ label, value, tip }) => (
  <Tooltip title={tip || ""}>
    <Chip label={`${label}: ${value ?? 0}`} size="small" sx={{ fontWeight: 600, bgcolor: "action.selected" }} />
  </Tooltip>
);
...
export default function DropDailyWidget() {
  ...
  async function handleDryRun() {
    ...
    try {
      ...
      console.error(e);
      setToast({ open: true, severity: "error", msg: "Dry-run failed. See console." });
    } finally {
      setDryBusy(false);
    }
  }
  ...
  return (
    <Card elevation={2} sx={{ borderRadius: 3 }}>
      <CardHeader
        ...
        title={<Typography variant="subtitle1" fontWeight={700}>Daily Drop Status</Typography>}
        subheader={
          <Stack direction="row" spacing={1} alignItems="center">
            <InfoOutlinedIcon fontSize="small" />
            <Typography variant="caption" color="text.secondary">Next schedule: {nextRunStatic}</Typography>
          </Stack>
        }
        action={isAdmin && (
          <Tooltip title="Preview what the drop will do (no writes)">
            <span>
              <Button size="small" variant="outlined" startIcon={<ScienceIcon />} onClick={handleDryRun} disabled={dryBusy}>
      ...
      <CardContent sx={{ pt: 1.5 }}>
        {loading ? (
          <Box><Skeleton width={220} /><Skeleton width="100%" height={38} /></Box>
        ) : (
          <Stack spacing={1.2}>
            <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap">
              <EventIcon fontSize="small" />
              <Typography variant="body2" fontWeight={600}>Last run:</Typography>
      ...
      <Dialog open={dryOpen} onClose={() => setDryOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent dividers>
          <Stack direction="row" spacing={1} useFlexGap flexWrap="wrap">
            ...
            <Stat label="Skipped (no TripID)" value={dryStats?.skippedNoTripId} />
      ...
      <Snackbar open={toast.open} autoHideDuration={4000}
        ...
        <Alert onClose={() => setToast((t) => ({ ...t, open: false }))}
          severity={toast.severity} variant="filled" sx={{ width: "100%" }}>
          {toast.msg}
        </Alert>
      </Snackbar>
    </Card>
