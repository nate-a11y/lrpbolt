...
import {
  Box, MenuItem, Select, Dialog, DialogTitle, DialogContent,
  DialogActions, Typography, TextField, Button, FormHelperText,
  FormControl, Tooltip, Grid, Snackbar, Alert, CircularProgress
} from '@mui/material';
...
const EditableRideGrid = ({ rows, onDelete, loading = false, refreshRides, sheetName = 'RideQueue' }) => {
  ...
  const logoUrl = "https://lakeridepros.xyz/Color%20logo%20-%20no%20background.png";
  ...
  const handleSave = async () => {
    ...
    const hasChanges = Object.entries(payload).some(([key, newVal]) => {
      const originalVal = key === 'RideDuration'
        ? selectedRow[key]
    ...
    if (!hasChanges) {
      setSnack({ open: true, message: '⚠️ No changes to save.', severity: 'info' });
      setSaving(false);
      return;
    }
      ...
      <Tooltip title="Reload ride data from Google Sheets">
        <span>
          <Button
            ...
            variant="outlined"
            size="small"
            startIcon={loading ? <CircularProgress size={16} sx={{ color: 'inherit' }} /> : <RefreshIcon />}
          >
    ...
    <Grid container spacing={2}>
      ...
      <Grid item xs={6}>
        <TextField
          ...
          onChange={(e) => setEditedRow({ ...editedRow, PickupTime: e.target.value })}
          InputLabelProps={{ shrink: true }}
      ...
      <Grid item xs={6}>
        <TextField
          ...
          onChange={(e) => setEditedRow({ ...editedRow, DurationHours: e.target.value })}
        />
      </Grid>
      <Grid item xs={6}>
        <TextField
          ...
          onChange={(e) => setEditedRow({ ...editedRow, DurationMinutes: e.target.value })}
        />
      </Grid>
      <Grid item xs={12}>
        <FormControl fullWidth error={!!validationErrors.RideType}>
          <Select
            ...
            onChange={(e) => setEditedRow({ ...editedRow, RideType: e.target.value })}
            displayEmpty
          >
            <MenuItem value="" disabled>Select Ride Type</MenuItem>
      ...
      <Grid item xs={12}>
        <FormControl fullWidth error={!!validationErrors.Vehicle}>
          <Select
            ...
            onChange={(e) => setEditedRow({ ...editedRow, Vehicle: e.target.value })}
            displayEmpty
          >
      ...
      <Grid item xs={12}>
        <TextField
          ...
          onChange={(e) => setEditedRow({ ...editedRow, RideNotes: e.target.value })}
        />
      </Grid>
    </Grid>
  </DialogContent>
...
<Typography>
  ...
  <span style={{ color: 'limegreen', fontWeight: 600 }}>{selectedRow.TripID}</span>
</Typography>

        <Typography><strong>Date:</strong> {selectedRow.Date}</Typography>
        <Typography><strong>Pickup Time:</strong> {selectedRow.PickupTime}</Typography>
        <Typography><strong>Duration:</strong> {selectedRow.RideDuration}</Typography>
        <Typography><strong>Ride Type:</strong> {selectedRow.RideType}</Typography>
        <Typography><strong>Vehicle:</strong> {selectedRow.Vehicle}</Typography>
        <Typography><strong>Notes:</strong> {selectedRow.RideNotes}</Typography>
        <Typography><strong>Created By:</strong> {selectedRow.CreatedBy}</Typography>
        <Typography><strong>Last Modified By:</strong> {selectedRow.LastModifiedBy}</Typography>
      </Box>
    ) : (
