...
export default function RideVehicleCalendar() {
  ...
  const getVehicleColor = (vehicle) => {
    const storedColors = JSON.parse(
      localStorage.getItem("vehicleColors") || "{}",
    );
    ...
    const color =
      index < BASE_COLORS.length
        ? baseColor
        : adjustColor(baseColor, adjustment);
    ...
    const fetchEvents = async () => {
      ...
      try {
        ...
        const parsed = (data.items || []).flatMap((item, i, all) => {
          ...
          const title =
            item.summary?.replace("(Lake Ride Pros)", "").trim() || "Untitled";
          ...
          const prev = all[i - 1];
          const prevEnd = prev
            ? dayjs.utc(prev.end.dateTime || prev.end.date).tz(CST)
            : null;
          const tightGap =
            prev &&
            start.diff(prevEnd, "minute") <= 10 &&
            vehicle === (prev.description?.match(/Vehicle:\s*(.+)/)?.[1] || "");
  ...
  const vehicleOptions = useMemo(
    () => ["ALL", ...[...new Set(events.map((e) => e.vehicle))]],
    [events],
  );
  const filteredEvents = useMemo(
    () =>
      vehicleFilter.includes("ALL")
        ? events
        : events.filter((e) => vehicleFilter.includes(e.vehicle)),
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs}>
      <PageContainer>
        ...
        <Typography fontSize={14} sx={{ mb: 1 }}>
          {summary.count} Rides • {summary.vehicles} Vehicles • {summary.tight}{" "}
          Tight Gaps
        </Typography>
