...
export const getAccessLevel = async (email) => {
  const json = await get(
    `${BASE_URL}?type=access&email=${encodeURIComponent(email)}`,
  );
...
export const fetchTicket = (ticketId) =>
  get(`${BASE_URL}?type=ticket&ticketId=${ticketId}`);

export const addTicket = (ticketData) =>
  post({ type: "addticket", ...ticketData });

export const deleteTicket = (ticketId) =>
  post({ type: "deleteticket", ticketId });
